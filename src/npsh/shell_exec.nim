...
proc executeOnHost*(host: Host, command: seq[string]): tuple[output: string, exitCode: int] =
...
proc executeOnHosts*(hosts: seq[Host], command: seq[string], prefixOutput: bool) =
  ...
  for host in hosts:
    let (output, exitCode) = executeOnHost(host, command)
